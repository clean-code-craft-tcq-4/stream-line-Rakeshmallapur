...
public class SenderServiceImpl implements SenderService {
	...
	List<SenderParameters> streamData = new ArrayList<SenderParameters>();
	...
	public List<SenderParameters> receiveReadingsFromBattery(int readingcount) {
		...
		for (int i = 1; i <= readingcount; i++) {
			...
			streamData.add(new SenderParameters(temperatureValue, SocValue, ChargeValue));
		}
		return streamData;
	}
	...
	public void sendReadingsToConsole() {
		...
		for (SenderParameters sendData : streamData) {
